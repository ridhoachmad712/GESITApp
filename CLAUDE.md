...
- [ ] Fase 1 — MVP (auth, CRUD dokumen, visibility, frontend publik, pencarian)
  ...
  - [x] Sesi 10 — Fungsi & tampilan (4 paket, dikonfirmasi user): (A) halaman error kustom 403/404/419/429 layout publik + 500/503 standalone tanpa DB, favicon SVG dinamis dari warna+inisial setting, upload logo di Pengaturan Tampilan (navbar/footer/brand Filament, fallback kotak inisial), hapus fitur hapus-akun-mandiri Breeze; (B) ActivityLogResource read-only (badge aksi, filter aksi/user/rentang tanggal) + widget EmptyCategories gap analysis (non-lazy; induk dianggap terisi jika sub punya dokumen); (C) halaman "Unggah Massal" (maks 30 file, metadata bersama, judul dari nama file, slug unik otomatis, validasi MIME+50MB, log per dokumen, metadata dipertahankan utk batch berikut — F2.1 SELESAI); (D) PDF.js self-hosted via pdfjs-dist+Vite (entry pdf-viewer.js + worker; CDN dihapus); 111 test pass (2026-06-11)
- [x] Fase 2 — Bulk upload, import user, statistik — SELESAI (2026-06-12): F2.1 bulk upload (Sesi 10), F2.2 impor pengguna Excel/CSV (halaman Filament "Impor Pengguna" + App\Services\UserImporter via openspout: header alias nama/email/nim-nip, deteksi delimiter ;, role per-batch, password awal = NIM atau tetap, laporan baris dilewati, maks 1000 baris), F2.3 statistik (gap analysis Sesi 10 + widget ActivityTrendChart unduhan-vs-dilihat 6 bulan dari activity_logs + tabel TopDownloadedDocuments; widget non-lazy agar terender server-side). F2.4 SOP pengisian = tugas administratif prodi, bukan kode. 155 test pass
- [ ] Fase 3 — Versioning, notifikasi MoU, mapping akreditasi LAMEMBA (F3.5 galeri dokumentasi SELESAI 2026-06-12: /dokumentasi grid foto public+published semua kategori + lightbox Alpine; route documents.image tanpa log/counter + cache 1 hari)
  - [x] F3.3 — Mapping akreditasi LAMEMBA SELESAI (2026-06-12): tabel accreditation_criteria (unique instrument+code) + pivot document_criteria, AccreditationCriteriaSeeder 9 kriteria LAMEMBA (K1–K9, editable), model AccreditationCriterion (relasi documents, accessor full_label), Document::criteria(); Filament: AccreditationCriterionResource (grup Manajemen Arsip, reorderable, tolak hapus jika ada bukti) + multi-select kriteria & filter di DocumentResource; halaman /bundel-akreditasi (auth+role:admin,dosen) accordion per kriteria dgn jumlah bukti (merah jika kosong) + aksi Lihat/Unduh/Buka, link nav Breeze "Bundel Akreditasi" (disembunyikan utk mahasiswa), chip kriteria di detail dokumen; 188 test pass
  - Catatan Sesi 13 (2026-06-12, penyempurnaan halaman depan menyeluruh): autocomplete /cari/saran (JSON 5 judul, Alpine searchSuggest di hero+hub+cari), filter kategori+tahun & highlight <mark> di /cari (SearchHighlighter, test judul pakai assertSeeText), bar pengumuman editable (announcement_* di Konten Beranda, tampil semua halaman publik), tombol Salin Tautan+WhatsApp di detail, sitemap.xml (cache 1 jam, public+published saja) + robots.txt + meta OG + JSON-LD (WebSite+SearchAction di beranda, CreativeWork di detail), cache data beranda 5 menit (HomeController::CACHE_KEY, bust via event saved/deleted Document+Category, increment counter TIDAK mem-bust), skip-link + tombol kembali-ke-atas, menu navbar Dokumentasi (nav_dokumentasi_order); 180 test pass
